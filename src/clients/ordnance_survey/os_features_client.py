...
class OSFeaturesClient:
    ...
    def fetch_all_usrn_features(
        self, 
        bbox: List[float], 
        count: int = 100,
        max_features: Optional[int] = None
    ) -> List[Tuple[str, Optional[datetime], str, str, float, float]]:
        """Fetch all USRN features within a bounding box with pagination.
        ...
        Returns:
            List of tuples: (usrn, version_date, source_product, geom_wkt, longitude, latitude)
        ...
        while True:
            logger.info(f"Fetching USRN page with offset {offset}")

            response = self._fetch_usrn_features_with_retry(bbox, count, offset)
            all_features.extend(response)

            logger.info(f"Received {len(response)} USRN features")

            # Check stopping conditions
            if len(response) < count:
                logger.info(f"Last page reached. Total: {len(all_features)} USRN features")
                break
            ...
            offset += count
            logger.info(f"{len(all_features)} USRN features found so far")
    ...
    def fetch_toids_for_usrn(
        ...
        """Fetch all TOID features for a specific USRN.
        
        Args:
            usrn: The USRN to fetch TOIDs for
            bbox: Optional bounding box to limit search area [min_lon, min_lat, max_lon, max_lat]
            count: Number of features per request (max 1000)
            max_features: Optional limit on total features to fetch
            
        Returns:
            List of tuples: (toid, version_date, source_product, geom_wkt, longitude, latitude)
            
        Raises:
            ValueError: If USRN is invalid or API returns error
            requests.RequestException: If API request fails
        """
        if not usrn or not isinstance(usrn, str):
            raise ValueError("USRN must be a non-empty string")
        ...
        while True:
            logger.info(f"Fetching TOIDs for USRN {usrn} with offset {offset}")

            response = self._fetch_toids_for_usrn_with_retry(usrn, bbox, count, offset)
            all_features.extend(response)

            logger.info(f"Received {len(response)} TOID features for USRN {usrn}")

            # Check stopping conditions
            if len(response) < count:
                logger.info(f"Last page reached. Total: {len(all_features)} TOID features for USRN {usrn}")
                break
            ...
            offset += count
            logger.info(f"{len(all_features)} TOID features found so far for USRN {usrn}")
    ...
    def _fetch_toids_for_usrn_with_retry(
        self,
        usrn: str,
        ...
        """Fetch TOIDs for USRN with retry logic."""
        for attempt in range(self.retry_attempts):
            try:
                return self._fetch_toids_for_usrn(usrn, bbox, count, start)
    ...
    def _fetch_toids_for_usrn(
        ...
        """Fetch a single page of TOID features for a specific USRN."""
        params = {
            "service": "WFS",
            "version": "2.0.0",
            "request": "GetFeature",
        ...
        # Add USRN filter using CQL_FILTER
        params["CQL_FILTER"] = f"USRN='{usrn}'"
    ...
    def _fetch_usrn_features_with_retry(
        self, 
        bbox: List[float], 
        count: int, 
        start: int
    ) -> List[Tuple[str, Optional[datetime], str, str, float, float]]:
        """Fetch USRN features with retry logic."""
        for attempt in range(self.retry_attempts):
            try:
                return self._fetch_usrn_features(bbox, count, start)
            except requests.RequestException as e:
                if attempt == self.retry_attempts - 1:
                    raise
                logger.warning(f"Request failed (attempt {attempt + 1}/{self.retry_attempts}): {e}")
    ...
    def _fetch_usrn_features(
        self, 
        bbox: List[float], 
        count: int, 
        start: int
    ) -> List[Tuple[str, Optional[datetime], str, str, float, float]]:
        """Fetch a single page of USRN features."""
        params = {
            "service": "WFS",
            "version": "2.0.0",
            "request": "GetFeature",
        ...
        logger.debug(f"Fetching {count} USRN features from index {start}")
        ...
        try:
            ...
            return self._parse_usrn_features(data["features"])
    ...
    def _parse_usrn_features(
        self, 
        features: List[dict]
    ) -> List[Tuple[str, Optional[datetime], str, str, float, float]]:
        """Parse USRN features from GeoJSON response."""
        data = []
        ...
        for i, feature in enumerate(features):
            try:
                ...
                # Extract required fields - USRN structure might be different from TOID
                usrn = props.get("USRN")
                version_date = props.get("VersionDate")
                source_product = props.get("SourceProduct", "OpenUSRN_USRN")
                ...
                if geom_type == "LineString" and isinstance(coordinates[0], list):
                    # For LineString, use the first coordinate pair
                    longitude = float(coordinates[0][0])
                    latitude = float(coordinates[0][1])
                elif geom_type == "MultiLineString" and isinstance(coordinates[0], list) and isinstance(coordinates[0][0], list):
                    # For MultiLineString, use the first coordinate of the first LineString
                    ...
                    logger.warning(f"Feature {i}: Unsupported geometry type: {geom_type}")
                    continue
                
                # Validate required fields
                if not usrn:
                    logger.warning(f"Feature {i}: Missing USRN")
                    continue
                
                # Parse date
                parsed_date = self._parse_version_date(version_date)
                
                # Create WKT geometry - for USRN this might be a LineString or MultiLineString
                if geom_type == "LineString":
                    coord_pairs = [f"{coord[0]} {coord[1]}" for coord in coordinates]
                    geom_wkt = f"LINESTRING({', '.join(coord_pairs)})"
                elif geom_type == "MultiLineString":
                    # For MultiLineString, create a MULTILINESTRING WKT
                    linestring_parts = []
                    for linestring_coords in coordinates:
                        coord_pairs = [f"{coord[0]} {coord[1]}" for coord in linestring_coords]
                        linestring_parts.append(f"({', '.join(coord_pairs)})")
                    geom_wkt = f"MULTILINESTRING({', '.join(linestring_parts)})"
                else:
                ...
                logger.warning(f"Error parsing USRN feature {i}: {e}")
                continue
                
        logger.debug(f"Parsed {len(data)} valid USRN features from {len(features)} total")
        return data
