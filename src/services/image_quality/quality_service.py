...
class ImageQualityService:
    ...
    def _calculate_overall_score(self, blur_score: float, exposure_score: float, 
                               size_score: float, road_percentage: float) -> float:
        """Calculate weighted overall quality score"""
        # Weighted combination of scores
        heuristic_score = (blur_score * 0.4 + exposure_score * 0.3 + size_score * 0.3)
        
        # Road surface bonus/penalty
        if road_percentage >= 25.0:
            road_bonus = min(20.0, road_percentage / 5.0)
        else:
            road_bonus = -30.0
